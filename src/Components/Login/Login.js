    ...
    const onSubmit = data =>{
        if(data){
            ...
            if(newUser && inputData.email && inputData.password){
                createUserWithEmailAndPassword(inputData.name, inputData.email, inputData.password)
                .then(res => {
                    handleResponse(res, true)
                });
            };
    ...
    return (
        <div className='container'>
            <div className="login_container">
                <Link to='/'><img className='signInLogo' src={logo} alt="Home"/></Link>
                   {
                       loggedInUser.isSignIn ?

                        <div className='account_container mt-5'>
                            <Link className='signIn_btn' onClick={signOut} to=''>Sign Out</Link>
                        </div>
                       :
                       ...
                       <>
                            {
                                ...
                                <CreateAccount onSubmit={onSubmit}  setNewUser={setNewUser} newUser={newUser}/>
                                ...
                                <>
                                    {
                                        ...
                                        <ForgotPassword onSubmit={onSubmit} setForgotPassword={setForgotPassword} forgotPassword={forgotPassword}/>
                                        
                                        :
                                        
                                        <EmailAndPassword onSubmit={onSubmit} setForgotPassword={setForgotPassword} forgotPassword={forgotPassword} setNewUser={setNewUser} newUser={newUser}/>
